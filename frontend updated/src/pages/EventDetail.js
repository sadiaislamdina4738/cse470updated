...
const EventDetail = () => {
  ...
  Array.isArray(event.waitlist)
    ? event.waitlist.findIndex((w) => String(w?._id ?? w) === uid)
    : -1;
  ...
  const hasPaymentLink = event.paymentLink && event.paymentLink.trim() !== '';

  const hasPaid = event.paidUsers?.some((u) => String(u) === uid);
          ...
          <div className="detail-item">
            ...
            {hasPaid ? (
             ...
             <button
               ...
               onClick={() => window.open(`/payment?eventId=${event._id}&amount=99.99&organizerName=${event.creator?.username || 'Organizer'}`,'_blank'
              )}>
                  Pay Now
            </button>
           )}
          </div>
          {isWaitlisted && waitlistPosition >=0 && (
  <div className="detail-item">
    <strong>Waitlist Position:</strong> #{waitlistPosition + 1}
  </div>
)}
      </div>
